  ...
  try {
    ...
    productCatalog.forEach((product, index) => {
      ...
      batch.set(docRef, {
        ...
        featured: index < 6,
        imageUrl: '', // Will be populated with actual images later
        createdAt: Timestamp.now(),
      })
    })

    await batch.commit()
    console.log(`✓ Successfully seeded ${productCatalog.length} products to Firestore!`)
    console.log(`✓ First 6 products marked as featured`)
    process.exit(0)
  } catch (error) {
